...
public class PBService {
    ...
    public PageDTO<PBResponse.PBPageDTO> getBookmarkPBs(MyUserDetails myUserDetails, Pageable pageable) {
        ...
        try {
            ...
            for (PBResponse.PBPageDTO pbPageDTO : pbPG) {
                ...
                pbPageDTO.setReserveCount(reservationRepository.countByPBIdAndProcess(pbId, ReservationProcess.COMPLETE));
                pbPageDTO.setReviewCount(reviewRepository.countByPBId(pbId));
    ...
    public PageDTO<PBResponse.PBPageDTO> getLoginSpecialityPBWithDistance(MyUserDetails myUserDetails, Double latitude, Double longitude, PBSpeciality speciality, Pageable pageable) {
        ...
        if (role.equals(Role.USER) || role.equals(Role.ADMIN)) {
            User user = userRepository.findById(id).orElseThrow(() -> new Exception404("존재하지 않는 회원입니다."));
            ...
            pbRepository.findById(id).orElseThrow(
    ...
    public PBResponse.PBProfileDTO getPBProfile(MyUserDetails myUserDetails, Long id) {
        ...
        if (myUserDetails.getMember().getRole().equals(Role.USER) || myUserDetails.getMember().equals(Role.ADMIN)) {
    ...
    public void updateProfile(MyUserDetails myUserDetails, PBRequest.UpdateProfileInDTO updateDTO, MultipartFile profileFile, MultipartFile portfolioFile) {
        ...
        if (profileFile != null && !profileFile.isEmpty()) {
            String profilePath = s3Util.upload(profileFile, "profile");
